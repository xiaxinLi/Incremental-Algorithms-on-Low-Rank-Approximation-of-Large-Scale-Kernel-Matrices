...
% t=min(length(SW>1e-14),desiredRank);%desireRank与SW中非0(小于1e-14看作0)的特征值
% SW = 1 ./ SW(1:t,:); SW=[SW;zeros(desiredRank-t,1)];
SW = 1 ./ SW(1:desiredRank,:);
UW = UW(:,1:desiredRank).* SW';
U =sqrt(m/(n1+n2)) * C* UW; % approximated by L * L'
end
